  ...
  return (
    <div className="min-h-screen bg-[#1e2a38] font-body text-[#FFB42D]">
      <div className="flex flex-col lg:flex-row min-h-screen">
        {/* Sidebar */}
        <aside className="w-full lg:w-80 bg-[#25303a] text-[#FFB42D] p-8 flex flex-col border-r border-[#FFB42D]/10">
          <div className="mb-12">
            <div className="flex items-center gap-4 mb-4">
              <div className="relative w-14 h-14">
                <div className="absolute inset-0 bg-[#FFB42D] rounded-2xl rotate-6 group-hover:rotate-12 transition-transform duration-500"></div>
                <div className="absolute inset-0 bg-[#1e2a38] rounded-2xl flex items-center justify-center shadow-lg border-2 border-[#FFB42D]/20">
                  <div className="relative">
                    <Factory className="text-[#FFB42D] w-7 h-7" />
                    <div className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-[#FFB42D] rounded-full border-2 border-[#1e2a38] animate-pulse"></div>
                  </div>
                </div>
              </div>
              <div>
                <h1 className="font-headline text-2xl tracking-tight leading-none text-[#FFB42D]">JMR<br/><span className="text-[#FFB42D]">Atelier</span></h1>
              </div>
            </div>
            <div className="h-[1px] w-full bg-gradient-to-r from-[#FFB42D]/40 to-transparent mb-4"></div>
            <p className="text-[10px] uppercase tracking-[0.4em] text-[#FFB42D]/40 font-bold">Excellence Textile</p>
          </div>
          ...
          <nav className="space-y-4 flex-1">
            <button 
              ...
              className={`w-full flex items-center justify-between p-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest border transition-all ${
                currentView === "dashboard" ? "bg-[#FFB42D]/10 border-[#FFB42D]/10 text-[#FFB42D]" : "hover:bg-white/5 border-transparent"
              }`}
            >
              <span className="flex items-center gap-3"><TrendingUp className="w-4 h-4 text-[#FFB42D]" /> Dashboard</span>
              <ChevronRight className="w-4 h-4" />
            </button>
            <button 
              onClick={() => setCurrentView("stock")}
              className={`w-full flex items-center justify-between p-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest border transition-all ${
                currentView === "stock" ? "bg-[#FFB42D]/10 border-[#FFB42D]/10 text-[#FFB42D]" : "hover:bg-white/5 border-transparent"
              }`}
            >
              <span className="flex items-center gap-3"><Package className="w-4 h-4 text-[#FFB42D]" /> Stocks</span>
              <ChevronRight className="w-4 h-4" />
            </button>
            <button 
              onClick={() => setCurrentView("tech-sheets")}
              className={`w-full flex items-center justify-between p-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest border transition-all ${
                currentView === "tech-sheets" ? "bg-[#FFB42D]/10 border-[#FFB42D]/10 text-[#FFB42D]" : "hover:bg-white/5 border-transparent"
              }`}
            >
              <span className="flex items-center gap-3"><FileText className="w-4 h-4 text-[#FFB42D]" /> Fiches Techniques</span>
              <ChevronRight className="w-4 h-4" />
            </button>
            <button 
              onClick={() => setCurrentView("qc")}
              className={`w-full flex items-center justify-between p-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest border transition-all ${
                currentView === "qc" ? "bg-[#FFB42D]/10 border-[#FFB42D]/10 text-[#FFB42D]" : "hover:bg-white/5 border-transparent"
              }`}
            >
              <span className="flex items-center gap-3"><ShieldCheck className="w-4 h-4 text-[#FFB42D]" /> Contrôle Qualité</span>
              <ChevronRight className="w-4 h-4" />
            </button>
            <button 
              onClick={() => setCurrentView("history")}
              className={`w-full flex items-center justify-between p-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest border transition-all ${
                currentView === "history" ? "bg-[#FFB42D]/10 border-[#FFB42D]/10 text-[#FFB42D]" : "hover:bg-white/5 border-transparent"
              }`}
            >
              <span className="flex items-center gap-3"><History className="w-4 h-4 text-[#FFB42D]" /> Historique</span>
              <ChevronRight className="w-4 h-4" />
            </button>
          </nav>

          <div className="mt-auto pt-8 border-t border-[#FFB42D]/10 space-y-4">
            <div className="flex items-center gap-4 p-4 bg-[#FFB42D]/5 rounded-2xl">
              <div className="w-10 h-10 rounded-full bg-[#FFB42D] flex items-center justify-center font-bold text-sm text-[#1e2a38]">
                OP
              </div>
              <div>
        ...
        <main className="flex-1 p-6 md:p-12 space-y-12 overflow-y-auto">
            ...
            <>
              ...
              <section className="max-w-5xl mx-auto space-y-8">
                <div className="flex justify-between items-end">
                  <div>
                    <h2 className="font-headline text-3xl text-[#FFB42D]">Suivi des Lignes</h2>
                    <p className="text-[10px] uppercase tracking-widest text-[#FFB42D]/40 font-bold mt-1">État des commandes en cours</p>
                  </div>
                  ...
                  {productionLines.map(line => (
                    <div key={line.id} className="bg-[#25303a] p-8 rounded-[2rem] shadow-sm border border-[#FFB42D]/5 space-y-6 group hover:shadow-2xl transition-all duration-500">
                      <div className="flex justify-between items-start">
                        <div>
                          <span className="text-[10px] font-bold uppercase tracking-widest text-[#FFB42D] mb-1 block">{line.order}</span>
                          <h3 className="font-headline text-2xl text-[#FFB42D]">{line.name}</h3>
                        </div>
                        <div className={`w-3 h-3 rounded-full ${
                          line.status === "en_cours" ? "bg-[#FFB42D] animate-pulse" : 
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-[#FFB42D]/40">
                          <span>Progression de la ligne</span>
                          <span className="text-[#FFB42D]">{line.progress}%</span>
                        </div>
                        <div className="h-2 w-full bg-[#FFB42D]/5 rounded-full overflow-hidden">
                          <div 
                            className={`h-full transition-all duration-700 ${line.status === "probleme" ? "bg-red-500" : "bg-[#FFB42D]"}`}
                      ...
                      <div className="pt-4 grid grid-cols-2 gap-4">
                        ...
                        <button 
                          ...
                          className="py-4 bg-[#FFB42D] text-[#1e2a38] text-[9px] font-bold uppercase tracking-widest rounded-2xl hover:bg-[#FFB42D] transition-all flex items-center justify-center gap-2 shadow-lg hover:shadow-xl active:scale-95"
                        >
                          Marquer Terminé
                        </button>
                      </div>
          ...
          {currentView === "history" && (
            <div className="flex flex-col items-center justify-center py-20 text-[#FFB42D]/20">
