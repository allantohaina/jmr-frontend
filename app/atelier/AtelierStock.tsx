  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="font-headline text-3xl text-[#FFB31B]">Gestion des Stocks</h2>
          <p className="text-[10px] uppercase tracking-widest text-[#FFB31B]/40 font-bold mt-1">Inventaire des matières premières</p>
        </div>
        <div className="relative w-full md:w-64">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-[#1e2a38] border border-[#FFB31B]/10 rounded-2xl text-sm text-[#FFB31B] focus:outline-none focus:ring-2 focus:ring-[#FFB31B]/20 transition-all placeholder:text-[#FFB31B]/20"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          <p className="text-sm text-[#FFB31B]/40 italic md:col-span-3">Chargement du stock…</p>
        ) : filteredItems.length === 0 ? (
          <p className="text-sm text-[#FFB31B]/40 italic md:col-span-3">Aucune matière en stock pour le moment.</p>
        ) : (
        <>
        {filteredItems.map(item => (
          <div key={item.id} className="bg-[#25303a] p-6 rounded-[2rem] border border-[#FFB31B]/5 shadow-sm hover:shadow-2xl transition-all group">
            ...
            <h3 className="font-headline text-xl text-[#FFB31B] mb-1">{item.name}</h3>
            <p className="text-[10px] uppercase tracking-widest text-[#FFB31B]/40 font-bold mb-6">{item.category}</p>

            <div className="flex items-center justify-between bg-[#1e2a38] p-4 rounded-2xl">
              <div>
                <p className="text-[9px] uppercase tracking-widest text-[#FFB31B]/40 font-bold">Quantité</p>
                <p className="text-lg font-bold text-[#FFB31B]">{item.quantity} <span className="text-xs font-medium opacity-60">{item.unit}</span></p>
              </div>
              <div className="flex gap-2">
                <button 
                  onClick={() => updateQuantity(item.id, -1)}
                  className="w-10 h-10 rounded-xl bg-[#25303a] border border-[#FFB31B]/10 text-[#FFB31B] flex items-center justify-center hover:bg-red-500/10 hover:text-red-400 transition-colors"
                >
                  <Minus className="w-4 h-4" />
                </button>
                <button 
                  onClick={() => updateQuantity(item.id, 1)}
                  className="w-10 h-10 rounded-xl bg-[#25303a] border border-[#FFB31B]/10 text-[#FFB31B] flex items-center justify-center hover:bg-green-500/10 hover:text-green-400 transition-colors"
                >
