  ...
  return (
    <div className="bg-[#161D30] rounded-[2rem] shadow-sm border border-[#EAA100]/5 overflow-hidden">
      <div className="p-8 border-b border-[#EAA100]/10 flex justify-between items-center bg-[#1e2a38]">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-[#EAA100]/10 flex items-center justify-center">
            <FileText className="text-[#EAA100] w-6 h-6" />
          </div>
          <div>
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-8">
              ...
              <div key={task.id} className="flex gap-4 items-start animate-in fade-in slide-in-from-right-4 duration-300" style={{ animationDelay: `${index * 100}ms` }}>
                <div className="flex-1 space-y-4">
                  <div className="relative">
                    <Package className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#EAA100]/20" />
                    <input
                      type="text"
                      placeholder="Description de la tâche..."
                      value={task.description}
                      onChange={(e) => updateTask(task.id, "description", e.target.value)}
                      className="w-full p-4 pl-12 bg-[#1e2a38] border border-[#EAA100]/10 rounded-2xl text-sm text-[#EAA100] focus:outline-none focus:ring-2 focus:ring-[#EAA100]/20 placeholder:text-[#EAA100]/20"
                    />
                  </div>
                  <div className="flex items-center gap-4">
                    <input
        ...
        <div className="pt-8 border-t border-[#EAA100]/10">
          <button
            type="submit"
