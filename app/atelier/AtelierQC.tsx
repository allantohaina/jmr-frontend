...
export function AtelierQC() {
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in duration-500">
      <div className="bg-[#25303a] rounded-[2.5rem] shadow-xl border border-[#FFB31B]/5 overflow-hidden">
        <div className="p-10 bg-[#FFB31B] text-[#1e2a38] flex justify-between items-center">
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-10">
          <div className="space-y-6">
            <div>
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#FFB31B]/40 mb-3 block">Numéro de Commande / Lot</label>
              <input
                type="text"
                value={orderId}
                onChange={(e) => setOrderId(e.target.value)}
                placeholder="Ex: #CMD-104-LOT2"
                className="w-full p-5 bg-[#1e2a38] border border-[#FFB31B]/10 rounded-2xl text-lg font-bold text-[#FFB31B] focus:outline-none focus:ring-2 focus:ring-[#FFB31B]/20 placeholder:text-[#FFB31B]/20"
              />
            </div>

            <div className="space-y-3">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#FFB31B]/40 mb-3 block">Checklist de Vérification</label>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-4 p-5 rounded-2xl border transition-all text-left ${
                      step.checked 
                        ...
                        : "bg-[#1e2a38] border-[#FFB31B]/5 text-[#FFB31B]/40 hover:border-[#FFB31B]/20"
                    }`}
                  >
                    {step.checked ? (
          ...
          <div className="pt-10 border-t border-[#FFB31B]/10 flex flex-col md:flex-row justify-between items-center gap-6">
