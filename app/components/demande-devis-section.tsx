...
function HelpDot({ tip }: { tip: string }) {
  return (
    <span className="relative inline-flex h-3.5 w-3.5 cursor-help items-center justify-center rounded-full border border-[#EAA100]/40 text-micro font-semibold not-italic tracking-normal text-[#EAA100]/60 group-hover:text-[#EAA100]">
      ?
      <span className="pointer-events-none absolute bottom-[calc(100%+6px)] left-1/2 z-10 hidden w-52 -translate-x-1/2 rounded-md border border-white/10 bg-[#161D30] px-3 py-2 text-label font-normal leading-relaxed normal-case tracking-normal text-[#EDF2F7] shadow-xl group-hover:block">
        {tip}
      </span>
    </span>
  );
...
function QuoteFormContent() {
  ...
  const inputClass = (hasError: boolean) =>
    `w-full rounded-md border bg-white px-4 py-3 text-body-lg text-[#1B2436] placeholder:text-[#B9C3D0] focus:border-[#F5C518] focus:outline-none focus:ring-2 focus:ring-[#F5C518]/30 transition-all [color-scheme:light] ${
      hasError ? "border-[#E05252]/60" : "border-[#D7DEE8]"
    }`;

  const fieldLabelClass = "flex items-center gap-1.5 text-caption font-bold uppercase tracking-widest text-[#B9C0CF] mb-2";
  ...
  return (
    <div className="min-h-screen bg-[#1e2a38] pb-16 pt-20 md:pb-24 md:pt-24">
      <div className="mx-auto w-full max-w-[920px] px-5" id="demande-devis-form">
        {/* PROGRESS */}
        <div className="sticky top-[88px] z-20 -mx-5 border-b border-[#EAA100]/10 bg-[#1e2a38]/95 px-5 py-3.5 backdrop-blur-sm">
          <div className="flex justify-center" aria-label="Progression du formulaire">
            {PROGRESS_STEPS.map((label, i) => (
              <div
                key={label}
                className={`relative flex items-center gap-2 px-3 text-label sm:px-4 ${
                  i === activeStep ? "text-[#EAA100]" : i < activeStep ? "text-[#EAA100]/70" : "text-[#8B94A3]"
                }`}
              >
                {i < PROGRESS_STEPS.length - 1 && (
                  <span aria-hidden="true" className="absolute right-0 top-1/2 h-px w-4 -translate-y-1/2 bg-white/15 sm:w-6" />
                )}
                <span
                  className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border font-mono text-caption transition-colors ${
                    i === activeStep
                      ? "border-[#F5C518] text-[#F5C518]"
                      : i < activeStep
                        ? "border-[#EAA100] bg-[#EAA100] text-[#1e2a38]"
                        : "border-white/15 text-[#8B94A3]"
                  }`}
                >
                  {i + 1}
                </span>
        ...
        <header className="mb-10 text-center md:mb-14">
          <span className="mb-4 block text-caption font-bold uppercase tracking-[0.3em] text-[#EAA100]">Demande de devis</span>
          <h2 className="mb-4 font-headline text-4xl font-semibold text-[#FFF8EC] md:text-5xl">
            {modifyCode ? `Modification du devis ${modifyCode}` : "Parlons de votre prochain projet textile"}
          </h2>
          <p className="mx-auto max-w-[560px] text-body-lg leading-relaxed text-[#B9C3D0]">
            {modifyCode
          ...
          <div className="mb-8 rounded-lg border border-amber-400/30 bg-amber-400/10 p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mb-4 text-sm text-[#B9C3D0]">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="rounded-lg border border-[#EAA100]/20 px-4 py-2 text-xs font-bold uppercase tracking-widest text-[#EAA100]/60 transition-colors hover:bg-white/5"
                  >
                    Annuler
                  </button>
                </div>
        ...
        {submitError ? (
          <div className="mb-8 rounded-lg border border-[#E05252]/20 bg-[#E05252]/10 px-5 py-4 text-sm text-red-100" role="alert">
            {submitError}
          </div>
        ) : null}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
              ...
              return (
                <button
                  ...
                  className={`rounded-md border-[1.5px] p-6 text-left transition-colors cursor-pointer ${
                    selected ? "border-[#F5C518] bg-[#F5C518]/[0.07]" : "border-white/10 bg-[#161D30] hover:border-[#F5C518]/50"
                  }`}
                >
                  <div className="mb-4 flex items-start justify-between">
                    <span className={`flex h-9 w-9 items-center justify-center rounded-md border text-base ${selected ? "border-[#F5C518] text-[#F5C518]" : "border-white/15 text-[#8B94A3]"}`}>
                      {card.icon}
                    </span>
                    <span className={`relative h-4 w-4 shrink-0 rounded-full border-[1.5px] ${selected ? "border-[#F5C518]" : "border-white/25"}`}>
                      {selected && <span className="absolute inset-[3px] rounded-full bg-[#F5C518]" />}
                    </span>
                  </div>
                  <span className="mb-1.5 block font-headline text-title font-semibold text-[#FFF8EC]">{card.name}</span>
                  <span className="mb-3 block text-body leading-relaxed text-[#B9C3D0]">{card.desc}</span>
                  <span className="flex flex-wrap gap-1.5">
                    {card.tags.map((tag) => (
                      <span key={tag} className="rounded-sm bg-white/5 px-2 py-1 text-caption text-[#8B94A3]">{tag}</span>
                    ))}
                  </span>
                </button>
              );
          ...
          <section ref={specsRef} className="mb-5 overflow-hidden rounded-lg border border-white/10 bg-[#161D30]">
            <div className="flex items-baseline gap-3 border-b border-white/10 px-6 py-5 md:px-7">
              <span className="font-mono text-xs text-[#EAA100]">01</span>
              <span className="font-headline text-lg font-semibold text-[#FFF8EC]">Spécifications du produit</span>
              <span className="ml-auto text-xs text-[#8B94A3]">{projectType === "serie" ? "Production en série" : "Pièce sur-mesure"}</span>
            </div>
            <div className="p-6 md:p-7">
              <div className="mb-6">
                ...
                <div className="relative">
                  <Controller
                    ...
                    render={({ field }) => (
                      <select
                        ...
                        className={`${inputClass(!!errors.category)} cursor-pointer appearance-none pr-10 ${errors.category ? "border-[#E05252]/50" : ""}`}
                      >
                  ...
                  />
                  <ChevronDown className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8B94A3]" />
                </div>
                {errors.category && <p className="mt-2 ui-error">{errors.category.message}</p>}
              </div>
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-label text-[#8B94A3]">Repère : léger 120-160 · standard 180-220 · épais 280+</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <Controller name="delai_souhaite" control={control} render={({ field }) => (
                    <input {...field} id="quote-delai" type="date" min={new Date().toISOString().split("T")[0]} className={`${inputClass(!!errors.delai_souhaite)} ${errors.delai_souhaite ? "border-[#E05252]/50" : ""}`} />
                  )} />
                  {errors.delai_souhaite && <p className="mt-2 ui-error">{errors.delai_souhaite.message}</p>}
                </div>
              </div>
            </div>
          </section>
          ...
          <section ref={docsRef} className="mb-5 overflow-hidden rounded-lg border border-white/10 bg-[#161D30]">
            <div className="flex items-baseline gap-3 border-b border-white/10 px-6 py-5 md:px-7">
              <span className="font-mono text-xs text-[#EAA100]">02</span>
              <span className="font-headline text-lg font-semibold text-[#FFF8EC]">Documents de référence</span>
              <span className="ml-auto text-xs text-[#8B94A3]">Facultatif</span>
            </div>
            <div className="p-6 md:p-7">
              <Controller
                ...
                render={({ field: { onChange, ref } }) => (
                  <label className="block cursor-pointer rounded-md border border-dashed border-white/25 p-6 text-center transition-colors hover:border-[#F5C518]" htmlFor="quote-technical-files">
                    <span className="mb-1 block text-body font-medium text-[#EDF2F7]">Croquis, photos d&apos;inspiration ou fiche technique</span>
                    <span className="mb-4 block text-label text-[#8B94A3]">JPG, PNG, WEBP, PDF — {MAX_ATTACHMENTS} fichiers max, 10 Mo par fichier</span>
                    <span className="inline-flex items-center gap-2 rounded-md bg-[#EAA100]/10 px-4 py-2.5 text-caption font-bold uppercase tracking-widest text-[#EAA100] transition-colors hover:bg-[#EAA100]/20">
                ...
                <div className="mt-4 space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {attachments.map((file, index) => (
                      <div key={`${file.name}-${file.lastModified}-${index}`} className="flex items-center gap-3 rounded-md border border-white/10 bg-white/5 p-3">
                        {isImageFile(file) ? (
                          <img src={URL.createObjectURL(file)} alt={file.name} className="h-10 w-10 shrink-0 rounded object-cover" />
                        ) : (
                          <FileImage className="h-9 w-9 shrink-0 text-[#F5C518]" />
                        )}
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-xs font-medium text-[#EDF2F7]" title={file.name}>{file.name}</p>
                          <p className="text-caption uppercase tracking-widest text-[#8B94A3]">{formatBytes(file.size)}</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => removeAttachment(index)}
                          className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[#8B94A3] transition-colors hover:bg-white/5 hover:text-[#F3A3A6]"
          ...
          <section ref={coordsRef} className="mb-5 overflow-hidden rounded-lg border border-white/10 bg-[#161D30]">
            <div className="flex items-baseline gap-3 border-b border-white/10 px-6 py-5 md:px-7">
              <span className="font-mono text-xs text-[#EAA100]">03</span>
              <span className="font-headline text-lg font-semibold text-[#FFF8EC]">Vos coordonnées</span>
            </div>
            <div className="p-6 md:p-7">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
                <div>
                  ...
                  {errors.name && <p className="mt-2 ui-error">{errors.name.message}</p>}
                </div>
                <div>
                  ...
                  {errors.email && <p className="mt-2 ui-error">{errors.email.message}</p>}
                </div>
                <div>
                ...
                <div>
                  ...
                  {modifyCode ? (
                    <p className="mt-1.5 text-caption uppercase tracking-widest text-[#8B94A3]">
                ...
                <div className="md:col-span-2">
                  ...
                  <Controller name="message" control={control} render={({ field }) => (
                    <textarea
                      ...
                      rows={5}
                      className={`${inputClass(!!errors.message)} min-h-[100px] resize-y ${errors.message ? "border-[#E05252]/50" : ""}`}
                    />
                  )} />
                  {errors.message && <p className="mt-2 ui-error">{errors.message.message}</p>}
                </div>
              </div>
            </div>
          </section>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <button
              ...
              className="flex w-full items-center justify-center gap-3 rounded-md border border-white/25 py-4 text-xs font-bold uppercase tracking-[0.3em] text-[#EAA100] transition-all hover:bg-[#EAA100]/10 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isSavingDraft ? (
                <>
            ...
            <button
              className="flex w-full items-center justify-center gap-3 rounded-md bg-[#EAA100] py-4 text-xs font-bold uppercase tracking-[0.3em] text-[#1e2a38] shadow-xl shadow-[#EAA100]/10 transition-all hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
              type="submit"
              disabled={isSubmitting}
            >
              {isSubmitting ? (
            ...
            {draft && (
                <button
                  ...
                  className="sm:col-span-2 flex w-full items-center justify-center gap-3 rounded-md border border-[#E05252]/30 py-3.5 text-xs font-bold uppercase tracking-[0.3em] text-[#F3A3A6] transition-all hover:bg-[#E05252]/10 disabled:cursor-not-allowed disabled:opacity-50"
                >
              ...
              {draft && (
                <p className="sm:col-span-2 text-center text-caption uppercase tracking-widest text-[#8B94A3]">
                  Vous modifiez un brouillon. Cliquez sur Envoyer pour l&apos;envoyer à notre équipe.
                </p>
              )}
            </div>

          <div className="mt-8 flex items-center justify-center gap-3 text-[#8B94A3]">
            <Check className="h-4 w-4 text-[#EAA100]" />
            <span className="text-label uppercase tracking-widest">Confidentialité garantie — vos documents et idées ne sont jamais partagés sans votre accord.</span>
          </div>
        </form>
      </div>
...
export function DemandeDevisSection() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-[#EAA100]">Chargement...</div>}>
      <QuoteFormContent />
    </Suspense>
  );
}