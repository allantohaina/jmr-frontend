...
function HelpDot({ tip }: { tip: string }) {
  return (
    <span className="relative inline-flex h-3.5 w-3.5 cursor-help items-center justify-center rounded-full border border-[#FFB31B]/40 text-[8px] font-semibold not-italic tracking-normal text-[#FFB31B]/60 group-hover:text-[#FFB31B]">
      ?
      <span className="pointer-events-none absolute bottom-[calc(100%+6px)] left-1/2 z-10 hidden w-52 -translate-x-1/2 rounded-md border border-[#FFB31B]/10 bg-[#25303a] px-3 py-2 text-[11px] font-normal leading-relaxed normal-case tracking-normal text-[#FFB31B]/80 shadow-xl group-hover:block">
        {tip}
      </span>
    </span>
  );
...
function QuoteFormContent() {
  ...
  const inputClass = (hasError: boolean) =>
    `w-full rounded-md border bg-[#1e2a38] px-4 py-3 text-[13.5px] text-[#FFB31B] placeholder:text-[#FFB31B]/25 focus:border-[#FFB31B] focus:outline-none transition-all ${
      hasError ? "border-red-400/50" : "border-[#FFB31B]/10"
    }`;

  const fieldLabelClass = "flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-[#FFB31B]/50 mb-2";
  ...
  return (
    <div className="min-h-screen bg-[#1e2a38] pb-16 pt-20 md:pb-24 md:pt-24">
      <div className="mx-auto w-full max-w-[920px] px-5" id="demande-devis-form">
        {/* PROGRESS */}
        <div className="sticky top-[88px] z-20 -mx-5 border-b border-[#FFB31B]/10 bg-[#1e2a38]/95 px-5 py-3.5 backdrop-blur-sm">
            ...
            {PROGRESS_STEPS.map((label, i) => (
              <div
                ...
                className={`relative flex items-center gap-2 px-3 text-[11px] sm:px-4 ${
                  i === activeStep ? "text-[#FFB31B]" : i < activeStep ? "text-[#FFB31B]/70" : "text-[#FFB31B]/35"
                }`}
              >
                {i < PROGRESS_STEPS.length - 1 && (
                  <span aria-hidden="true" className="absolute right-0 top-1/2 h-px w-4 -translate-y-1/2 bg-[#FFB31B]/15 sm:w-6" />
                )}
                <span
                  className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border font-mono text-[10px] transition-colors ${
                    i === activeStep
                      ? "border-[#FFB31B] text-[#FFB31B]"
                      : i < activeStep
                        ? "border-[#FFB31B] bg-[#FFB31B] text-[#1e2a38]"
                        : "border-[#FFB31B]/15 text-[#FFB31B]/35"
                  }`}
                >
                  {i + 1}
                </span>
        ...
        <header className="mb-10 text-center md:mb-14">
          <span className="mb-4 block text-[10px] font-bold uppercase tracking-[0.3em] text-[#FFB31B]">Demande de devis</span>
          <h2 className="mb-4 font-headline text-4xl font-semibold text-[#FFB31B] md:text-5xl">
            {modifyCode ? `Modification du devis ${modifyCode}` : "Parlons de votre prochain projet textile"}
          </h2>
          <p className="mx-auto max-w-[560px] text-[14.5px] leading-relaxed text-[#FFB31B]/60">
            {modifyCode
          ...
          <div className="mb-8 rounded-lg border border-amber-400/30 bg-amber-400/10 p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mb-4 text-sm text-[#FFB31B]/70">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="rounded-lg border border-[#FFB31B]/20 px-4 py-2 text-xs font-bold uppercase tracking-widest text-[#FFB31B]/60 transition-colors hover:bg-white/5"
                  >
                    Annuler
                  </button>
                </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
              ...
              return (
                <button
                  ...
                  className={`rounded-md border-[1.5px] p-6 text-left transition-colors cursor-pointer ${
                    selected ? "border-[#FFB31B] bg-[#25303a]" : "border-[#FFB31B]/10 bg-[#25303a] hover:border-[#FFB31B]/60"
                  }`}
                >
                  <div className="mb-4 flex items-start justify-between">
                    <span className={`flex h-9 w-9 items-center justify-center rounded-md border text-base ${selected ? "border-[#FFB31B] text-[#FFB31B]" : "border-[#FFB31B]/20 text-[#FFB31B]"}`}>
                      {card.icon}
                    </span>
                    <span className={`relative h-4 w-4 shrink-0 rounded-full border-[1.5px] ${selected ? "border-[#FFB31B]" : "border-[#FFB31B]/25"}`}>
                      {selected && <span className="absolute inset-[3px] rounded-full bg-[#FFB31B]" />}
                    </span>
                  </div>
                  <span className="mb-1.5 block font-headline text-[17px] font-semibold text-[#FFB31B]">{card.name}</span>
                  <span className="mb-3 block text-[12.5px] leading-relaxed text-[#FFB31B]/60">{card.desc}</span>
                  <span className="flex flex-wrap gap-1.5">
                    {card.tags.map((tag) => (
                      <span key={tag} className="rounded-sm bg-[#1e2a38] px-2 py-1 text-[10px] text-[#FFB31B]/45">{tag}</span>
                    ))}
                  </span>
                </button>
              );
          ...
          <section ref={specsRef} className="mb-5 overflow-hidden rounded-lg border border-[#FFB31B]/10 bg-[#25303a]">
            <div className="flex items-baseline gap-3 border-b border-[#FFB31B]/10 px-6 py-5 md:px-7">
              <span className="font-mono text-xs text-[#FFB31B]">01</span>
              <span className="font-headline text-lg font-semibold text-[#FFB31B]">Spécifications du produit</span>
              <span className="ml-auto text-xs text-[#FFB31B]/50">{projectType === "serie" ? "Production en série" : "Pièce sur-mesure"}</span>
            </div>
            <div className="p-6 md:p-7">
              <div className="mb-6">
                ...
                <div className="relative">
                  ...
                  />
                  <ChevronDown className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#FFB31B]" />
                </div>
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-[11px] text-[#FFB31B]/40">Repère : léger 120-160 · standard 180-220 · épais 280+</p>
                </div>
                <div>
          ...
          <section ref={docsRef} className="mb-5 overflow-hidden rounded-lg border border-[#FFB31B]/10 bg-[#25303a]">
            <div className="flex items-baseline gap-3 border-b border-[#FFB31B]/10 px-6 py-5 md:px-7">
              <span className="font-mono text-xs text-[#FFB31B]">02</span>
              <span className="font-headline text-lg font-semibold text-[#FFB31B]">Documents de référence</span>
              <span className="ml-auto text-xs text-[#FFB31B]/50">Facultatif</span>
            </div>
            <div className="p-6 md:p-7">
              <Controller
                ...
                render={({ field: { onChange, ref } }) => (
                  <label className="block cursor-pointer rounded-md border border-dashed border-[#FFB31B]/25 p-6 text-center transition-colors hover:border-[#FFB31B]" htmlFor="quote-technical-files">
                    <span className="mb-1 block text-[13px] font-medium text-[#FFB31B]/80">Croquis, photos d&apos;inspiration ou fiche technique</span>
                    <span className="mb-4 block text-[11px] text-[#FFB31B]/45">JPG, PNG, WEBP, PDF — {MAX_ATTACHMENTS} fichiers max, 10 Mo par fichier</span>
                    <span className="inline-flex items-center gap-2 rounded-md bg-[#FFB31B]/10 px-4 py-2.5 text-[10px] font-bold uppercase tracking-widest text-[#FFB31B] transition-colors hover:bg-[#FFB31B]/20">
              ...
              {attachments.length > 0 ? (
                <div className="mt-4 space-y-2">
                  <p className="text-xs text-[#FFB31B]/60">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {attachments.map((file, index) => (
                      <div key={`${file.name}-${file.lastModified}-${index}`} className="flex items-center gap-3 rounded-md border border-[#FFB31B]/10 bg-[#1e2a38]/70 p-3">
                        {isImageFile(file) ? (
                          <img src={URL.createObjectURL(file)} alt={file.name} className="h-10 w-10 shrink-0 rounded object-cover" />
                        ) : (
                          <FileImage className="h-9 w-9 shrink-0 text-[#FFB31B]" />
                        )}
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-xs font-medium text-[#FFB31B]" title={file.name}>{file.name}</p>
                          <p className="text-[10px] uppercase tracking-widest text-[#FFB31B]/40">{formatBytes(file.size)}</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => removeAttachment(index)}
                          className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[#FFB31B]/50 transition-colors hover:bg-white/5 hover:text-red-300"
          ...
          <section ref={coordsRef} className="mb-5 overflow-hidden rounded-lg border border-[#FFB31B]/10 bg-[#25303a]">
            <div className="flex items-baseline gap-3 border-b border-[#FFB31B]/10 px-6 py-5 md:px-7">
              <span className="font-mono text-xs text-[#FFB31B]">03</span>
              <span className="font-headline text-lg font-semibold text-[#FFB31B]">Vos coordonnées</span>
            </div>
            <div className="p-6 md:p-7">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
                <div>
                ...
                <div>
                  ...
                  {modifyCode ? (
                    <p className="mt-1.5 text-[10px] uppercase tracking-widest text-[#FFB31B]/40">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <button
              ...
              className="flex w-full items-center justify-center gap-3 rounded-md border border-[#FFB31B]/25 py-4 text-xs font-bold uppercase tracking-[0.3em] text-[#FFB31B] transition-all hover:bg-[#FFB31B]/10 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isSavingDraft ? (
                <>
            ...
            <button
              className="flex w-full items-center justify-center gap-3 rounded-md bg-[#FFB31B] py-4 text-xs font-bold uppercase tracking-[0.3em] text-[#1e2a38] shadow-xl shadow-[#FFB31B]/10 transition-all hover:bg-[#FFB31B] disabled:cursor-not-allowed disabled:opacity-50"
              type="submit"
              disabled={isSubmitting}
            >
              {isSubmitting ? (
            ...
            {draft && (
              ...
              {draft && (
                <p className="sm:col-span-2 text-center text-[10px] uppercase tracking-widest text-[#FFB31B]/50">
                  Vous modifiez un brouillon. Cliquez sur Envoyer pour l&apos;envoyer à notre équipe.
                </p>
              )}
            </div>

          <div className="mt-8 flex items-center justify-center gap-3 text-[#FFB31B]/40">
            <Check className="h-4 w-4 text-[#FFB31B]" />
...
export function DemandeDevisSection() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-[#FFB31B]">Chargement...</div>}>
      <QuoteFormContent />
    </Suspense>
  );
}