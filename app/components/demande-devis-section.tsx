...
function HelpDot({ tip }: { tip: string }) {
  return (
    <span className="relative inline-flex h-3.5 w-3.5 cursor-help items-center justify-center rounded-full border border-[#EAA100]/40 text-[8px] font-semibold not-italic tracking-normal text-[#EAA100]/60 group-hover:text-[#EAA100]">
      ?
      <span className="pointer-events-none absolute bottom-[calc(100%+6px)] left-1/2 z-10 hidden w-52 -translate-x-1/2 rounded-md border border-[#EAA100]/10 bg-[#25303a] px-3 py-2 text-[11px] font-normal leading-relaxed normal-case tracking-normal text-[#EAA100]/80 shadow-xl group-hover:block">
        {tip}
      </span>
    </span>
  );
...
function QuoteFormContent() {
  ...
  const inputClass = (hasError: boolean) =>
    `w-full rounded-md border bg-[#1e2a38] px-4 py-3 text-[13.5px] text-[#EAA100] placeholder:text-[#EAA100]/25 focus:border-[#EAA100] focus:outline-none transition-all ${
      hasError ? "border-red-400/50" : "border-[#EAA100]/10"
    }`;

  const fieldLabelClass = "flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-[#EAA100]/50 mb-2";
    ...
    <div className="min-h-screen bg-[#1e2a38] pb-16 pt-20 md:pb-24 md:pt-24">
      <div className="mx-auto w-full max-w-[920px] px-5" id="demande-devis-form">
            ...
            {PROGRESS_STEPS.map((label, i) => (
              <div
                ...
                className={`relative flex items-center gap-2 px-3 text-[11px] sm:px-4 ${
                  i === activeStep ? "text-[#EAA100]" : i < activeStep ? "text-[#EAA100]/70" : "text-[#EAA100]/35"
                }`}
              >
                {i < PROGRESS_STEPS.length - 1 && (
                  <span aria-hidden="true" className="absolute right-0 top-1/2 h-px w-4 -translate-y-1/2 bg-[#EAA100]/15 sm:w-6" />
                )}
                <span
                  className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border font-mono text-[10px] transition-colors ${
                    i === activeStep
                      ? "border-[#EAA100] text-[#EAA100]"
                      : i < activeStep
                        ? "border-[#EAA100] bg-[#EAA100] text-[#1e2a38]"
                        : "border-[#EAA100]/15 text-[#EAA100]/35"
                  }`}
                >
                  {i + 1}
                </span>
        ...
        <header className="mb-10 text-center md:mb-14">
          <span className="mb-4 block text-[10px] font-bold uppercase tracking-[0.3em] text-[#EAA100]">Demande de devis</span>
          <h2 className="mb-4 font-headline text-4xl font-semibold text-[#EAA100] md:text-5xl">
            {modifyCode ? `Modification du devis ${modifyCode}` : "Parlons de votre prochain projet textile"}
          </h2>
          <p className="mx-auto max-w-[560px] text-[14.5px] leading-relaxed text-[#EAA100]/60">
            {modifyCode
          ...
          <div className="mb-8 rounded-lg border border-amber-400/30 bg-amber-400/10 p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mb-4 text-sm text-[#EAA100]/70">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
              ...
              return (
                <button
                  ...
                  className={`rounded-md border-[1.5px] p-6 text-left transition-colors cursor-pointer ${
                    selected ? "border-[#EAA100] bg-[#25303a]" : "border-[#EAA100]/10 bg-[#25303a] hover:border-[#EAA100]/60"
                  }`}
                >
                  <div className="mb-4 flex items-start justify-between">
                    <span className={`flex h-9 w-9 items-center justify-center rounded-md border text-base ${selected ? "border-[#EAA100] text-[#EAA100]" : "border-[#EAA100]/20 text-[#EAA100]"}`}>
                      {card.icon}
                    </span>
                    <span className={`relative h-4 w-4 shrink-0 rounded-full border-[1.5px] ${selected ? "border-[#EAA100]" : "border-[#EAA100]/25"}`}>
                      {selected && <span className="absolute inset-[3px] rounded-full bg-[#EAA100]" />}
                    </span>
                  </div>
                  <span className="mb-1.5 block font-headline text-[17px] font-semibold text-[#EAA100]">{card.name}</span>
                  <span className="mb-3 block text-[12.5px] leading-relaxed text-[#EAA100]/60">{card.desc}</span>
                  <span className="flex flex-wrap gap-1.5">
                    {card.tags.map((tag) => (
                      <span key={tag} className="rounded-sm bg-[#1e2a38] px-2 py-1 text-[10px] text-[#EAA100]/45">{tag}</span>
                    ))}
                  </span>
                </button>
              );
          ...
          <section ref={specsRef} className="mb-5 overflow-hidden rounded-lg border border-[#EAA100]/10 bg-[#25303a]">
            <div className="flex items-baseline gap-3 border-b border-[#EAA100]/10 px-6 py-5 md:px-7">
              <span className="font-mono text-xs text-[#EAA100]">01</span>
              <span className="font-headline text-lg font-semibold text-[#EAA100]">Spécifications du produit</span>
              <span className="ml-auto text-xs text-[#EAA100]/50">{projectType === "serie" ? "Production en série" : "Pièce sur-mesure"}</span>
            </div>
            <div className="p-6 md:p-7">
              <div className="mb-6">
                ...
                <div className="relative">
                  ...
                  />
                  <ChevronDown className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#EAA100]" />
                </div>
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-[11px] text-[#EAA100]/40">Repère : léger 120-160 · standard 180-220 · épais 280+</p>
                </div>
                <div>
          ...
          <section ref={docsRef} className="mb-5 overflow-hidden rounded-lg border border-[#EAA100]/10 bg-[#25303a]">
            <div className="flex items-baseline gap-3 border-b border-[#EAA100]/10 px-6 py-5 md:px-7">
              <span className="font-mono text-xs text-[#EAA100]">02</span>
              <span className="font-headline text-lg font-semibold text-[#EAA100]">Documents de référence</span>
              <span className="ml-auto text-xs text-[#EAA100]/50">Facultatif</span>
            </div>
            <div className="p-6 md:p-7">
              <Controller
                ...
                render={({ field: { onChange, ref } }) => (
                  <label className="block cursor-pointer rounded-md border border-dashed border-[#EAA100]/25 p-6 text-center transition-colors hover:border-[#EAA100]" htmlFor="quote-technical-files">
                    <span className="mb-1 block text-[13px] font-medium text-[#EAA100]/80">Croquis, photos d&apos;inspiration ou fiche technique</span>
                    <span className="mb-4 block text-[11px] text-[#EAA100]/45">JPG, PNG, WEBP, PDF — {MAX_ATTACHMENTS} fichiers max, 10 Mo par fichier</span>
                    <span className="inline-flex items-center gap-2 rounded-md bg-[#EAA100]/10 px-4 py-2.5 text-[10px] font-bold uppercase tracking-widest text-[#EAA100] transition-colors hover:bg-[#EAA100]/20">
              ...
              {attachments.length > 0 ? (
                <div className="mt-4 space-y-2">
                  <p className="text-xs text-[#EAA100]/60">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {attachments.map((file, index) => (
                      <div key={`${file.name}-${file.lastModified}-${index}`} className="flex items-center gap-3 rounded-md border border-[#EAA100]/10 bg-[#1e2a38]/70 p-3">
                        {isImageFile(file) ? (
                          <img src={URL.createObjectURL(file)} alt={file.name} className="h-10 w-10 shrink-0 rounded object-cover" />
                        ) : (
                          <FileImage className="h-9 w-9 shrink-0 text-[#EAA100]" />
                        )}
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-xs font-medium text-[#EAA100]" title={file.name}>{file.name}</p>
                          <p className="text-[10px] uppercase tracking-widest text-[#EAA100]/40">{formatBytes(file.size)}</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => removeAttachment(index)}
                          className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[#EAA100]/50 transition-colors hover:bg-white/5 hover:text-red-300"
          ...
          <section ref={coordsRef} className="mb-5 overflow-hidden rounded-lg border border-[#EAA100]/10 bg-[#25303a]">
            <div className="flex items-baseline gap-3 border-b border-[#EAA100]/10 px-6 py-5 md:px-7">
              <span className="font-mono text-xs text-[#EAA100]">03</span>
              <span className="font-headline text-lg font-semibold text-[#EAA100]">Vos coordonnées</span>
            </div>
            <div className="p-6 md:p-7">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
                <div>
                ...
                <div>
                  ...
                  {modifyCode ? (
                    <p className="mt-1.5 text-[10px] uppercase tracking-widest text-[#EAA100]/40">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <button
              ...
              className="flex w-full items-center justify-center gap-3 rounded-md border border-[#EAA100]/25 py-4 text-xs font-bold uppercase tracking-[0.3em] text-[#EAA100] transition-all hover:bg-[#EAA100]/10 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isSavingDraft ? (
                <>
            ...
            <button
              className="flex w-full items-center justify-center gap-3 rounded-md bg-[#EAA100] py-4 text-xs font-bold uppercase tracking-[0.3em] text-[#1e2a38] shadow-xl shadow-[#EAA100]/10 transition-all hover:bg-[#EAA100] disabled:cursor-not-allowed disabled:opacity-50"
              type="submit"
              disabled={isSubmitting}
            >
              {isSubmitting ? (
            ...
            {draft && (
              ...
              {draft && (
                <p className="sm:col-span-2 text-center text-[10px] uppercase tracking-widest text-[#EAA100]/50">
                  Vous modifiez un brouillon. Cliquez sur Envoyer pour l&apos;envoyer à notre équipe.
                </p>
              )}
            </div>

          <div className="mt-8 flex items-center justify-center gap-3 text-[#EAA100]/40">
