    ...
    <article className="document-sheet mx-auto w-full max-w-[210mm] overflow-hidden bg-[#fffdf8] text-[#172d42] shadow-[0_24px_80px_rgba(3,24,43,.18)] print:max-w-none print:shadow-none">
      <header className="relative overflow-hidden bg-[#172d42] px-7 pb-8 pt-7 text-white sm:px-10">
        ...
        <div className="relative flex flex-col justify-between gap-8 sm:flex-row sm:items-start">
          <div className="flex flex-col gap-1">
            ...
            <p className="mt-1 text-[9px] font-bold uppercase tracking-[.24em] text-[#EAA100]">Atelier · Madagascar</p>
          </div>
          <div className="text-left sm:text-right">
            <p className="text-[10px] font-bold uppercase tracking-[.28em] text-[#EAA100]">Document opérationnel</p>
      ...
      <main className="p-7 sm:p-10">
        <section className="grid gap-6 border-b border-[#172d42]/15 pb-7 sm:grid-cols-[1fr_auto]">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#EAA100]">Destinataire</p>
        ...
        <section className="mt-7 grid gap-7 sm:grid-cols-[1fr_245px]">
          <div className="space-y-4 text-sm leading-6 text-[#40566a]">
            {deliveryAddress ? <div><p className="text-[9px] font-bold uppercase tracking-[.16em] text-[#EAA100]">Lieu de livraison</p><p className="mt-1 whitespace-pre-line">{deliveryAddress}</p></div> : null}
            {notes ? <div className="border-l-2 border-[#EAA100] bg-[#f4ead4]/45 px-4 py-3"><p className="text-[9px] font-bold uppercase tracking-[.16em] text-[#EAA100]">Notes</p><p className="mt-1 whitespace-pre-line">{notes}</p></div> : null}
        ...
        {signature ? <section className="mt-9 max-w-xs border border-[#172d42]/15 bg-[#faf6ec] p-4"><p className="text-[9px] font-bold uppercase tracking-[.16em] text-[#EAA100]">Approbation interne</p><p className="mt-3 font-serif text-2xl italic text-[#172d42]">{signature.name}</p><p className="mt-2 text-xs text-[#40566a]">Confirmé le {date(signature.signedAt)}</p></section> : null}
      </main>

      <footer className="border-t border-[#172d42]/15 px-7 py-5 text-[10px] leading-5 text-[#6f8292] sm:px-10"><div className="flex flex-col justify-between gap-2 sm:flex-row"><p className="whitespace-pre-line">{companyInfo.name}{companyInfo.address ? ` · ${companyInfo.address}` : ""}</p><p>{companyInfo.email}</p></div><p className="mt-3 text-[#EAA100]">Ce document est édité par JMR Textile. Toute validation interne affichée ici n’est pas une signature électronique qualifiée.</p></footer>
