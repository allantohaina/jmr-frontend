...
export function MobileSidebar({ isStaff, userFirstName }: MobileDrawerProps) {
  ...
  return (
    <>
      <button
        onClick={openDrawer}
        className="relative z-50 flex h-10 w-10 items-center justify-center rounded-xl transition-colors hover:bg-[#FFB31B]/10 md:hidden"
        aria-label="Ouvrir le menu"
      >
        {open ? (
          <X className="h-5 w-5 text-[#FFB31B]" />
        ) : (
          <Menu className="h-5 w-5 text-[#FFB31B]" />
        )}
      </button>
      ...
      >
        <div className="flex items-center justify-between border-b border-[#FFB31B]/10 px-6 py-5">
          <span className="font-headline text-lg font-bold tracking-wide text-[#FFB31B]">
            JMR Textile
          </span>
          <button
            onClick={closeDrawer}
            className="flex h-10 w-10 items-center justify-center rounded-xl transition-colors hover:bg-[#FFB31B]/10"
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-6">
                  ...
                  <Link
                    ...
                    className={`flex items-center gap-4 rounded-xl px-4 py-3.5 text-sm font-bold uppercase tracking-widest transition-all ${
                      active
                        ...
                        : "text-[#FFB31B]/70 hover:bg-[#FFB31B]/5 hover:text-[#FFB31B]"
                    }`}
                  >
                    <Icon className={`h-5 w-5 ${active ? "text-[#FFB31B]" : "text-[#FFB31B]/40"}`} />
                    {label}
                  </Link>
                </li>
              );
          ...
          {(isStaff || userFirstName) && (
            <>
              <div className="my-6 border-t border-[#FFB31B]/10" />
              <p className="mb-3 px-4 text-[10px] font-bold uppercase tracking-widest text-[#FFB31B]/40">
                Administration
              </p>
              <ul className="space-y-1">
                {isStaff && (
                  <li>
                    <Link
                      href="/backoffice"
                      onClick={closeDrawer}
                      className="flex items-center gap-4 rounded-xl px-4 py-3.5 text-sm font-bold uppercase tracking-widest text-[#FFB31B]/70 transition-all hover:bg-[#FFB31B]/5 hover:text-[#FFB31B]"
                    >
                      <Settings className="h-5 w-5 text-[#FFB31B]/40" />
                      Backoffice
                    </Link>
                  </li>
                )}
                <li>
                  <Link
                    ...
                    className="flex items-center gap-4 rounded-xl px-4 py-3.5 text-sm font-bold uppercase tracking-widest text-[#FFB31B]/70 transition-all hover:bg-[#FFB31B]/5 hover:text-[#FFB31B]"
                  >
                    <User className="h-5 w-5 text-[#FFB31B]/40" />
        ...
        <div className="border-t border-[#FFB31B]/10 px-4 py-6">
          <div className="space-y-3">
            {SECONDARY_LINKS.map(({ href, label, icon: Icon }) => (
              <Link
                ...
                style={{ background: "#FFB31B", color: "#1e2a38" }}
              >
          ...
          <p className="mt-6 text-center text-[10px] text-[#FFB31B]/30">
