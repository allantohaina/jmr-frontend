...
export function DocumentPreview({ file }: { file: FileAttachment }) {
  ...
  if (!isPdf || !mounted || !url) return null;

  return (
    <div className="overflow-hidden rounded-xl border border-[#163526]/10 bg-[#faf9f4] p-2">
      {error ? (
        <a className="flex items-center gap-2 p-3 text-xs font-semibold text-[#163526] underline" href={url} target="_blank" rel="noopener noreferrer">
          <FileText className="h-4 w-4" /> Ouvrir le PDF
        </a>
      ) : (
        <Suspense fallback={<Loader2 className="m-5 h-5 w-5 animate-spin text-[#EAA100]" />}>
          <PdfDocument file={url} loading={<Loader2 className="m-5 h-5 w-5 animate-spin text-[#EAA100]" />} onLoadSuccess={({ numPages }) => setPages(numPages)} onLoadError={() => setError(true)}>
  ...
  return (
    <div className="rounded-xl border border-[#163526]/10 bg-[#faf9f4] p-3">
      <button type="button" onClick={readCsv} className="inline-flex items-center gap-2 text-xs font-bold text-[#163526] underline">
        <FileSpreadsheet className="h-4 w-4 text-[#EAA100]" /> Prévisualiser le CSV
      </button>
