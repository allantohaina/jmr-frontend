  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#163526]/10 bg-[#faf9f4] p-2">
      {error ? (
        ...
        <Suspense fallback={<Loader2 className="m-5 h-5 w-5 animate-spin text-[#FFB31B]" />}>
          <PdfDocument file={url} loading={<Loader2 className="m-5 h-5 w-5 animate-spin text-[#FFB31B]" />} onLoadSuccess={({ numPages }) => setPages(numPages)} onLoadError={() => setError(true)}>
  ...
  return (
    <div className="rounded-xl border border-[#163526]/10 bg-[#faf9f4] p-3">
      <button type="button" onClick={readCsv} className="inline-flex items-center gap-2 text-xs font-bold text-[#163526] underline">
        <FileSpreadsheet className="h-4 w-4 text-[#FFB31B]" /> Prévisualiser le CSV
      </button>
