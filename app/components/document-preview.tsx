    ...
    <div className="overflow-hidden rounded-xl border border-[#163526]/10 bg-[#faf9f4] p-2">
      {error ? (
        ...
        <Suspense fallback={<Loader2 className="m-5 h-5 w-5 animate-spin text-[#e5ad46]" />}>
          <PdfDocument file={url} loading={<Loader2 className="m-5 h-5 w-5 animate-spin text-[#e5ad46]" />} onLoadSuccess={({ numPages }) => setPages(numPages)} onLoadError={() => setError(true)}>
