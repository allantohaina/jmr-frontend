...
export default function ExchangeRateWidget() {
  ...
  if (error) {
    return (
      <div className="rounded-xl border border-[#FFB31B]/10 bg-[#25303a] p-6">
        <div className="mb-3 flex items-center gap-2">
          <CircleDollarSign className="text-[#FFB31B]" />
          <h3 className="font-headline text-lg text-[#f3e9d6]">Taux de Change</h3>
        </div>
        <p className="text-xs text-[#b14255]">Erreur de chargement</p>
        <button
          onClick={refresh}
          className="mt-3 rounded-lg border border-[#FFB31B]/15 px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-[#FFB31B] transition-colors hover:bg-[#FFB31B]/10"
        >
          Réessayer
        </button>
      </div>
  ...
  return (
    <div className="rounded-xl border border-[#FFB31B]/10 bg-[#25303a] p-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          onClick={refresh}
          className="rounded-lg p-2 transition-colors hover:bg-[#FFB31B]/10"
          title="Rafraîchir"
        >
      ...
      <div className="space-y-4">
        {isLoading && !rates ? (
          ...
          <>
            <div className="rounded-xl border border-[#FFB31B]/5 bg-[#26313d] p-4">
