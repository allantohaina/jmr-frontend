    ...
    <div className="space-y-4">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${
          dragActive
            ...
            : "border-[#FFB31B]/10 bg-[#1e2a38] hover:border-[#FFB31B]/10/30"
        }`}
      >
        <input
          type="file"
        ...
        >
          <Upload className={`mx-auto h-10 w-10 ${dragActive ? "text-orange-500" : "text-[#FFB31B]/40"}`} />
          <p className="mt-2 text-sm font-medium text-[#FFB31B]">
            {dragActive ? "Déposez les fichiers ici" : "Cliquez ou déposez pour ajouter des pièces jointes"}
          </p>
          <p className="text-[10px] text-[#FFB31B]/40 uppercase tracking-widest mt-1">
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-4">
          <Loader2 className="h-5 w-5 animate-spin text-[#FFB31B]" />
        </div>
      ) : attachments.length > 0 ? (
        <div className="space-y-2">
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#FFB31B]/40">
          ...
          <div className="space-y-2">
            {attachments.map((att) => (
              <div
                key={att.id}
                className="flex items-center gap-3 p-3 bg-[#25303a] border border-[#FFB31B]/10 rounded-xl hover:border-[#FFB31B]/10/20 transition-colors"
              >
                <div className="flex-shrink-0">{getFileIcon(att.mime_type)}</div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[#FFB31B] truncate">{att.original_name}</p>
                  <p className="text-[10px] text-[#FFB31B]/40 uppercase tracking-widest">
                ...
                <a
                  ...
                  className="p-2 text-[#FFB31B]/60 hover:text-orange-500 transition-colors"
                  title="Ouvrir"
                >
