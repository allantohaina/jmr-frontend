  ...
  return (
    <div className="px-4 md:px-12 py-8 md:py-10 space-y-8">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="font-headline text-2xl md:text-3xl text-[#F5A623]">Fiches Produits</h2>
          <p className="text-xs font-bold uppercase tracking-widest text-[#F5A623]/40 mt-1">Référentiel des produits de l&apos;atelier</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-[#25303a] rounded-2xl p-6 md:p-8 border border-[#F5A623]/10 shadow-sm">
          <h3 className="font-headline text-xl text-[#F5A623] mb-6">{editingId ? "Modifier" : "Créer"} un produit</h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Nom du produit *</label>
              <input
                required
                value={formData.nom}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Catégorie</label>
              <input
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Conso par taille (JSON)</label>
              <input
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Niveau difficulté défaut *</label>
              <input
                required
                type="number"
                step="0.1"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">MOQ *</label>
              <input
                required
                type="number"
                value={formData.moq}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Coût matière défaut (Ar/m) *</label>
              <input
                required
                type="number"
                step="0.01"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Photo URL</label>
              <input
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Description</label>
              <textarea
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                className="w-full bg-[#1e2a38] border-none p-3 rounded-xl text-xs font-bold text-[#F5A623] outline-none focus:ring-2 focus:ring-[#163526]/10 h-24"
            ...
            <div className="md:col-span-2 flex gap-3 pt-4">
              ...
              <button
                type="button"
                onClick={resetForm}
                className="px-6 py-3 border border-[#F5A623]/10 text-[#F5A623] font-bold text-[10px] uppercase tracking-widest rounded-xl hover:bg-[#F5A623]/10 transition-all"
              >
                Annuler
              </button>
            </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center rounded-2xl border border-[#F5A623]/10 bg-[#25303a] py-16">
          <Loader2 className="mr-3 h-5 w-5 animate-spin text-[#F5A623]/50" />
          Chargement...
        </div>
      ) : (
        <div className="bg-[#25303a] rounded-2xl border border-[#F5A623]/10 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-[#F5A623]/10 bg-[#F5A623]/10">
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Nom</th>
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Catégorie</th>
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Conso tissu</th>
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Difficulté</th>
                  <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">MOQ</th>
                  <th className="px-6 py-4 text-right text-[10px] font-bold uppercase tracking-widest text-[#F5A623]/40">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#F5A623]/10">
                {produits.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-sm italic text-[#F5A623]/40">
...
produits.map((p) => (
                      ...
                      <td className="px-6 py-4">
                        <p className="text-sm font-bold text-[#F5A623]">{p.nom}</p>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs text-[#F5A623]/60">{p.categorie || "—"}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-mono text-[#F5A623]">{p.conso_tissu_unitaire} m</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-mono text-[#F5A623]">x{p.niveau_difficulte_defaut}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-mono text-[#F5A623]">{p.moq}</span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <button
                            onClick={() => handleEdit(p)}
                            className="p-2 rounded-lg border border-[#F5A623]/10 text-[#F5A623] hover:border-orange-500 hover:text-orange-500 transition-colors"
                            title="Modifier"
                          >
       ...
       {selectedProduitId && (
         <div className="bg-[#25303a] rounded-2xl border border-[#F5A623]/10 shadow-sm overflow-hidden mt-6">
           <div className="p-6 border-b border-[#F5A623]/10 flex justify-between items-center">
             <div>
               <h3 className="font-headline text-xl text-[#F5A623]">Détails du produit</h3>
               <p className="text-xs text-[#F5A623]/40 uppercase tracking-widest mt-1">
