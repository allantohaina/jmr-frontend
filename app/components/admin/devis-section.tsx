...
const TABS: TabDef[] = [
  ...
  },
  {
    ...
    icon: FilePenLine,
    color: "text-[#FFB31B]/60",
    bg: "bg-[#25303a] border-[#FFB31B]/10",
...
function statusBadgeClass(status?: string | null): string {
  switch (status) {
    case "draft":
      return "bg-[#25303a] text-[#FFB31B]/70 border-[#FFB31B]/10 border";
    case "sent":
    ...
    default:
      return "bg-[#FFB31B]/10 text-[#FFB31B] border-[#FFB31B]/10 border";
  }
}
  ...
  return (
    <div className="space-y-8 px-4 py-8 sm:px-6 md:px-12 md:py-10">
      <div className="flex flex-col gap-3 px-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="font-headline text-2xl text-[#FFB31B] sm:text-3xl">
            Cotations &amp; Devis
          </h2>
          <p className="mt-1 text-xs font-bold uppercase tracking-widest text-[#FFB31B]/40">
            ...
            <button
              ...
              className={`text-left rounded-2xl border px-4 py-4 transition-all ${
                isActive
                  ...
                  : "bg-[#25303a] border-[#FFB31B]/10 text-[#FFB31B]/60 hover:bg-[#25303a]/80 hover:border-[#FFB31B]/20"
              }`}
            >
              <div className="flex items-center justify-between">
                <Icon
                  className={`h-4 w-4 ${
                    isActive ? tab.color : "text-[#FFB31B]/30"
                  }`}
                />
                <span
                  className={`text-[11px] font-bold rounded-full px-2 py-0.5 ${
                    isActive ? `${tab.bg} ${tab.color}` : "bg-[#1e2a38] text-[#FFB31B]/60"
                  }`}
                >
                  {c}
                </span>
              ...
              <p
                className={`mt-3 text-[11px] font-bold uppercase tracking-widest ${
                  isActive ? tab.color : "text-[#FFB31B]/70"
                }`}
              >
                {tab.label}
              </p>
              <p className="mt-0.5 text-[9px] font-bold uppercase tracking-widest text-[#FFB31B]/30">
                {tab.subtitle}
              </p>
            </button>
          );
      ...
      {isLoading ? (
        <div className="flex items-center justify-center rounded-[2rem] border border-[#FFB31B]/10 bg-[#25303a] py-10 md:py-16 text-[#FFB31B]/50">
        ...
        <>
          ...
          <div className="grid gap-4 md:hidden">
            {displayedQuotes.length > 0 ? (
              displayedQuotes.map((quote) => (
                <article
                  key={quote.id}
                  className="rounded-2xl border border-[#FFB31B]/10 bg-[#25303a] p-5 shadow-sm"
                >
                  <div className="mb-4 flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-[#FFB31B]">
                        {quote.titre || quote.name || "—"}
                      </p>
                      <p className="truncate text-xs text-[#FFB31B]/60">
                        {quote.email || "—"}
                      </p>
                      <p className="truncate text-[10px] text-[#FFB31B]/40">
                        {quote.phone || "—"}
                      </p>
                    </div>
                    <span
                  ...
                  <p className="line-clamp-2 text-xs leading-6 text-[#FFB31B]/60">
                    {quote.message || "—"}
                  </p>
                  {quote.amount != null &&
                  quote.amount !== "" &&
                  Number(quote.amount) > 0 ? (
                    <p className="mt-3 text-sm font-bold text-[#FFB31B]">
                      {Number(quote.amount).toLocaleString()} Ar
                    </p>
                  ) : (
                    <Link
                      href={`/backoffice/devis/edit?id=${quote.id}`}
                      className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-[#FFB31B]/70 hover:text-[#FFB31B]"
                    >
                  ...
                  <div className="mt-5 flex flex-wrap items-center gap-3">
                    ...
                    {quote.status === "accepted" && (
                      <button
                        ...
                        className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-[#FFB31B] hover:text-[#FFB31B] disabled:opacity-50"
                      >
              ...
              <div className="rounded-2xl border border-dashed border-[#FFB31B]/10 bg-[#25303a] p-8 text-center">
                <FileText className="mx-auto mb-3 h-8 w-8 text-[#FFB31B]/20" />
                <p className="text-sm italic text-[#FFB31B]/40">
          ...
          {/* Desktop: tableau */}
          <div className="hidden overflow-hidden rounded-[2rem] border border-[#FFB31B]/10 bg-[#25303a] shadow-sm md:block">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left">
                <thead>
                  <tr className="border-b border-[#FFB31B]/10 bg-[#1e2a38]">
                    <th className="px-8 py-6 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-[#FFB31B]/40">
                      Client
                    </th>
                    <th className="px-8 py-6 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-[#FFB31B]/40">
                      Contact
                    ...
                    <th className="px-8 py-6 text-right font-label text-[10px] font-bold uppercase tracking-[0.2em] text-[#FFB31B]/40">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#FFB31B]/10">
                  {displayedQuotes.length > 0 ? (
                    displayedQuotes.map((quote) => (
                      ...
                      >
                        <td className="px-8 py-6">
                          <p className="text-sm font-bold text-[#FFB31B]">
                            {quote.titre || quote.name || "—"}
                          </p>
                          {quote.created_at ? (
                            <p className="text-[10px] text-[#FFB31B]/40 mt-0.5">
                              Créé le{" "}
                              {new Date(
                                quote.created_at
                        ...
                        <td className="px-8 py-6">
                          <p className="text-xs text-[#FFB31B]">
                            {quote.email || "—"}
                          </p>
                          <p className="text-[10px] text-[#FFB31B]/40">
                            {quote.phone || "—"}
                          </p>
                        </td>
                        <td className="px-8 py-6">
                          <p className="line-clamp-1 max-w-xs text-xs text-[#FFB31B]/60">
                            {quote.message ||
                              (quote.category
                        ...
                        <td className="px-8 py-6">
                          ...
                          Number(quote.amount) > 0 ? (
                            ...
                            <Link
                              href={`/backoffice/devis/edit?id=${quote.id}`}
                              className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-[#FFB31B]/70 hover:text-[#FFB31B]"
                            >
                              Voir détail
                          ...
                          <div className="flex items-center justify-end gap-4">
                            {quote.status === "accepted" && (
                              <button
                                onClick={() => convertToCommande(quote)}
                                disabled={convertingId === quote.id}
                                className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-[#FFB31B] hover:text-[#FFB31B] disabled:opacity-50"
                              >
                                {convertingId === quote.id ? (
                                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                                ) : null}
                            ...
                            <Link
                              href={`/backoffice/devis/edit?id=${quote.id}`}
                              className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-[#FFB31B] hover:text-[#FFB31B]"
                            >
                              Gérer
                              <ChevronRight className="h-3.5 w-3.5" />
                            </Link>
                    ...
                    <tr>
                      ...
                      >
                        <FileText className="mx-auto mb-3 h-8 w-8 text-[#FFB31B]/20" />
                        <p className="font-body text-sm italic text-[#FFB31B]/40">
