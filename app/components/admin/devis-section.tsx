...
function statusBadgeClass(status?: string | null): string {
  switch (status) {
    ...
    default:
      return "bg-[#EAA100]/10 text-[#EAA100] border-[#EAA100]/10 border";
  }
}
  ...
  return (
    <div className="space-y-8 px-4 py-8 sm:px-6 md:px-12 md:py-10">
      <div className="flex flex-col gap-3 px-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="font-headline text-2xl text-[#EAA100] sm:text-3xl">
            Cotations &amp; Devis
          </h2>
          <p className="mt-1 text-xs font-bold uppercase tracking-widest text-[#EAA100]/40">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <Icon
                  className={`h-4 w-4 ${
                    isActive ? tab.color : "text-[#EAA100]/30"
                  }`}
                />
                <span
                ...
                >
                  <div className="mb-4 flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-[#EAA100]">
                  ...
                  <p className="line-clamp-2 text-xs leading-6 text-[#EAA100]/60">
                    {quote.message || "—"}
                  </p>
                  {quote.amount != null &&
                  quote.amount !== "" &&
                  Number(quote.amount) > 0 ? (
                    <p className="mt-3 text-sm font-bold text-[#EAA100]">
                      {Number(quote.amount).toLocaleString()} Ar
                    </p>
                  ) : (
                    <Link
                      href={`/backoffice/devis/edit?id=${quote.id}`}
                      className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-[#EAA100]/70 hover:text-[#EAA100]"
                    >
                ...
                <tbody className="divide-y divide-[#EAA100]/10">
                  {displayedQuotes.length > 0 ? (
                    displayedQuotes.map((quote) => (
                      ...
                      >
                        <td className="px-8 py-6">
                          <p className="text-sm font-bold text-[#EAA100]">
                            {quote.titre || quote.name || "—"}
                          </p>
                          {quote.created_at ? (
                        ...
                        <td className="px-8 py-6">
                          <p className="line-clamp-1 max-w-xs text-xs text-[#EAA100]/60">
                            {quote.message ||
                              (quote.category
                    ...
                    <tr>
                      ...
                      >
                        <FileText className="mx-auto mb-3 h-8 w-8 text-[#EAA100]/20" />
                        <p className="font-body text-sm italic text-[#EAA100]/40">
