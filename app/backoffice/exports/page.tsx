...
export default function ExportsPage() {
  ...
  return (
    <div className="px-6 md:px-12 py-10 space-y-8">
      <div className="px-2">
        <h2 className="font-headline text-3xl text-[#FFB31B]">Exports comptables</h2>
        <p className="text-[#FFB31B]/40 text-xs font-bold uppercase tracking-widest mt-1">Rapports CSV · JMR Atelier</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {EXPORTS.map((e) => (
          <div key={e.key} className="bg-[#25303a] rounded-2xl border border-[#FFB31B]/10 p-6 shadow-sm flex flex-col">
            <p className="font-headline text-lg text-[#FFB31B]">{e.label}</p>
            <p className="text-xs text-[#FFB31B]/50 mt-1 flex-1">{e.desc}</p>
            <button
              onClick={() => download(e.key, e.label)}
              disabled={busy !== null}
              className="mt-4 inline-flex items-center justify-center gap-2 rounded-xl bg-[#163526] px-4 py-2.5 text-[10px] font-bold uppercase tracking-widest text-[#FFB31B] hover:bg-[#1e4234] transition-colors disabled:opacity-50"
            >
