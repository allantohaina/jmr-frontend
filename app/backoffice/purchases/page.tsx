  ...
  return (
    <div className="px-6 md:px-12 py-10 space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="font-headline text-3xl text-[#EAA100] mb-2">Gestion des Achats</h1>
          <p className="text-xs uppercase tracking-widest text-[#EAA100]/40 font-bold">Suivi des dépenses et fournisseurs</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-[#161D30] p-6 rounded-[2rem] border border-[#EAA100]/10 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input required placeholder="Fournisseur" value={formData.fournisseur} onChange={(e) => setFormData({...formData, fournisseur: e.target.value})} className="w-full px-4 py-3 bg-[#1e2a38] border border-[#EAA100]/10 rounded-xl text-sm" />
            <select value={formData.categorie} onChange={(e) => setFormData({...formData, categorie: e.target.value})} className="w-full px-4 py-3 bg-[#1e2a38] border border-[#EAA100]/10 rounded-xl text-sm">
              {CATEGORIES_ACHAT.map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
            <input type="number" step="0.01" required placeholder="Montant" value={formData.montant} onChange={(e) => setFormData({...formData, montant: Number(e.target.value)})} className="w-full px-4 py-3 bg-[#1e2a38] border border-[#EAA100]/10 rounded-xl text-sm" />
            <input type="date" required value={formData.date_achat} onChange={(e) => setFormData({...formData, date_achat: e.target.value})} className="w-full px-4 py-3 bg-[#1e2a38] border border-[#EAA100]/10 rounded-xl text-sm" />
            <select value={formData.statut} onChange={(e) => setFormData({...formData, statut: e.target.value})} className="w-full px-4 py-3 bg-[#1e2a38] border border-[#EAA100]/10 rounded-xl text-sm">
      ...
      <div className="bg-[#161D30] rounded-[2.5rem] border border-[#EAA100]/10 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-[#EAA100]/40">Chargement...</td></tr>
              ) : filteredPurchases.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-[#EAA100]/40 italic">Aucun achat enregistré.</td></tr>
              ) : (
                filteredPurchases.map((p) => (
                  <tr key={p.id} className="border-b border-[#EAA100]/10 hover:bg-[#1e2a38]/50 transition-colors">
                    <td className="px-6 py-4">
                      <span className="text-xs text-[#EAA100]/60 font-mono">{p.date_achat}</span>
                    </td>
                    <td className="px-6 py-4">
