    ...
    <div className="p-6 md:p-10 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <DollarSign className="h-6 w-6 text-[#FFB31B]" />
          <h1 className="font-headline text-2xl text-[#FFB31B]">Paie</h1>
        </div>
      ...
      {payments.length > 0 && (
        <div className="rounded-xl bg-[#25303a] p-4 border border-[#FFB31B]/20">
          <p className="text-xs text-[#FFB31B] uppercase tracking-widest font-bold">Total</p>
          <p className="text-3xl font-bold text-[#FFB31B] mt-1">{total.toFixed(2)} €</p>
        </div>
      )}

      {showForm && (
        <div className="rounded-xl bg-[#25303a] p-5 border border-[#FFB31B]/20 space-y-4">
          <input value={form.employee} onChange={(e) => setForm({ ...form, employee: e.target.value })} placeholder="Nom de l'employé" className="w-full rounded-lg bg-[#1e2a38] px-4 py-3 text-sm text-[#FFB31B] placeholder:text-[#FFB31B]/30 outline-none border border-[#FFB31B]/10 focus:border-[#FFB31B]/30" />
          ...
          <button onClick={addPayment} className="w-full rounded-lg bg-[#FFB31B] py-3 text-[10px] font-bold uppercase tracking-widest text-[#1e2a38] transition hover:bg-[#FFB31B]">Ajouter</button>
        </div>
      )}

      <div className="relative">
        <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[#FFB31B]/30" />
        <input value={search} onChange={(e) => { setSearch(e.target.value); onSearchChange(e.target.value); }} placeholder="Rechercher un employé..." className="w-full rounded-xl bg-[#25303a] pl-11 pr-4 py-3 text-sm text-[#FFB31B] placeholder:text-[#FFB31B]/30 outline-none border border-[#FFB31B]/10 focus:border-[#FFB31B]/30" />
      </div>

      {filteredPayments.length === 0 ? (
        <p className="text-sm text-[#FFB31B]/50">{search ? "Aucun paiement trouvé." : "Aucun paiement enregistré."}</p>
      ) : (
        <div className="space-y-3">{filteredPayments.map((p, i) => (
          <div key={i} className="rounded-xl bg-[#25303a] p-4 border border-[#FFB31B]/10">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-semibold text-[#FFB31B]">{p.employee}</p>
                <p className="text-xs text-[#FFB31B]/50">{p.date}</p>
              </div>
