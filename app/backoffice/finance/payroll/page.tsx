...
export default function PayrollPage() {
  ...
  return (
    <div className="p-6 md:p-10 space-y-6">
      ...
      {payments.length > 0 && (
        <div className="rounded-xl bg-[#25303a] p-4 border border-[#EAA100]/20">
      ...
      {showForm && (
        <div className="rounded-xl bg-[#25303a] p-5 border border-[#EAA100]/20 space-y-4">
          ...
          <button onClick={addPayment} className="w-full rounded-lg bg-[#EAA100] py-3 text-[10px] font-bold uppercase tracking-widest text-[#1e2a38] transition hover:bg-[#EAA100]">Ajouter</button>
        </div>
      )}

      <div className="relative">
        <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[#EAA100]/30" />
        <input value={search} onChange={(e) => { setSearch(e.target.value); onSearchChange(e.target.value); }} placeholder="Rechercher un employé..." className="w-full rounded-xl bg-[#25303a] pl-11 pr-4 py-3 text-sm text-[#EAA100] placeholder:text-[#EAA100]/30 outline-none border border-[#EAA100]/10 focus:border-[#EAA100]/30" />
      </div>
