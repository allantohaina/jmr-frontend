...
export default function SiteContentPage() {
  ...
  const allImageFields = useMemo(() => sections.flatMap((s) => s.fields.filter((f) => f.image).map((f) => ({ ...f, section: s.title }))), [sections]);
  const [dragOver, setDragOver] = useState<string | null>(null);
  function handleDragOver(e: React.DragEvent, key: string) { e.preventDefault(); setDragOver(key); }
  function handleDragLeave(e: React.DragEvent) { e.preventDefault(); setDragOver(null); }
  async function handleDrop(e: React.DragEvent, field: Field) {
    e.preventDefault(); setDragOver(null);
    const file = e.dataTransfer.files?.[0]; if (!file || !field.image) return;
    // réutilise upload logique via faux event
    const fakeEvent = { target: { files: [file], value: "" } } as unknown as ChangeEvent<HTMLInputElement>;
    await upload(field, fakeEvent);
  }
  ...
  return <form onSubmit={save} className="mx-auto max-w-6xl px-6 py-8 md:px-12 md:py-12">
    ...
    {!loading && (
      <div className="rounded-2xl border border-[#e5ad46]/15 bg-[#25303a] p-5 md:p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="flex items-center gap-2 font-headline text-lg text-[#eccc90]"><ImagePlus className="h-5 w-5 text-[#e5ad46]" />Toutes les images — édition rapide</h3>
          <span className="text-[10px] font-bold uppercase tracking-widest text-[#eccc90]/40">{allImageFields.length} images • cliquer ou glisser pour modifier</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {allImageFields.map((field) => (
            <label
              ...
              className={`group relative overflow-hidden rounded-xl border-2 bg-[#1e2a38] cursor-pointer transition-all ${dragOver === field.key ? "border-[#e5ad46] scale-[1.02]" : "border-[#e5ad46]/20 hover:border-[#e5ad46]/50"} ${uploading === field.key ? "opacity-60" : ""}`}
            >
              <div className="aspect-[4/3] relative">
                <img src={value(field)} alt={field.label} className="h-full w-full object-cover" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition flex items-center justify-center opacity-0 group-hover:opacity-100">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-[#e5ad46] px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-[#1e2a38]">{uploading === field.key ? <Loader2 className="h-3 w-3 animate-spin" /> : <ImagePlus className="h-3 w-3" />} Modifier</span>
                </div>
                {dragOver === field.key && <div className="absolute inset-0 bg-[#e5ad46]/20 flex items-center justify-center"><span className="text-xs font-bold text-[#e5ad46]">Déposer ici</span></div>}
              </div>
              <div className="p-2.5">
                <p className="text-[10px] font-bold uppercase tracking-widest text-[#eccc90]/70 truncate">{field.label}</p>
                <p className="text-[9px] text-[#eccc90]/30 truncate">{field.section}</p>
              </div>
              <input type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" className="hidden" onChange={(e) => upload(field, e)} />
              {uploading === field.key && <div className="absolute top-2 right-2"><Loader2 className="h-4 w-4 animate-spin text-[#e5ad46]" /></div>}
            </label>
          ))}
        </div>
        <p className="mt-3 text-[10px] text-[#eccc90]/40">Astuce : vous pouvez aussi modifier chaque image directement dans sa section ci-dessous — aperçu cliquable, glisser-déposer, ou URL.</p>
      </div>
    )}
    {loading ? <div className="flex min-h-72 items-center justify-center gap-3 text-sm text-[#eccc90]/60"><Loader2 className="h-5 w-5 animate-spin" />Chargement du contenu…</div> : <div className="space-y-4">{sections.map((section, index) => <details key={section.title} open={index === 0} className="group overflow-hidden rounded-2xl border border-[#e5ad46]/15 bg-[#25303a] shadow-lg"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-5 marker:hidden md:px-7"><div><h3 className="font-headline text-xl text-[#eccc90]">{section.title}</h3><p className="mt-1 text-xs text-[#eccc90]/55">{section.description}</p></div><ChevronDown className="h-5 w-5 text-[#e5ad46] transition group-open:rotate-180" /></summary><div className="grid gap-5 border-t border-[#e5ad46]/10 p-5 md:grid-cols-2 md:p-7">{section.fields.map((field) => <div key={field.key} className={`block ${field.multiline || field.image ? "md:col-span-2" : ""}`}><span className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-[#eccc90]/75">{field.label}</span>{field.image ? <div className="grid gap-4 sm:grid-cols-[12rem_1fr]"><label onDragOver={(e) => handleDragOver(e, field.key)} onDragLeave={handleDragLeave} onDrop={(e) => handleDrop(e, field)} className={`relative aspect-[4/3] overflow-hidden rounded-xl border-2 bg-[#1e2a38] cursor-pointer group/img transition ${dragOver === field.key ? "border-[#e5ad46] scale-[1.02]" : "border-[#e5ad46]/15 hover:border-[#e5ad46]/40"} ${uploading === field.key ? "opacity-60" : ""}`}><img src={value(field)} alt={`Aperçu : ${field.label}`} className="h-full w-full object-cover" /><span className="absolute bottom-0 left-0 right-0 bg-black/55 px-2 py-1 text-[9px] font-bold uppercase tracking-widest text-[#eccc90]">Aperçu — cliquer ou glisser pour modifier</span><div className="absolute inset-0 bg-black/0 group-hover/img:bg-black/30 transition flex items-center justify-center opacity-0 group-hover/img:opacity-100"><span className="inline-flex items-center gap-1.5 rounded-full bg-[#e5ad46] px-3 py-1.5 text-[10px] font-bold text-[#1e2a38]"><ImagePlus className="h-3 w-3" /> Changer</span></div><input type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" className="hidden" onChange={(e) => upload(field, e)} />{dragOver === field.key && <div className="absolute inset-0 bg-[#e5ad46]/20 flex items-center justify-center"><span className="text-xs font-bold text-[#e5ad46]">Déposer ici</span></div>}</label><div className="flex flex-col justify-center gap-3"><input value={value(field)} onChange={(event) => setValue(field.key, event.target.value)} className="w-full rounded-xl border border-[#e5ad46]/20 bg-[#1e2a38] px-3 py-2.5 text-sm text-[#eccc90] outline-none focus:border-[#e5ad46]" aria-label={`URL de ${field.label}`} /><div className="flex flex-wrap gap-2"><label className="inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-lg border border-[#e5ad46]/30 px-3 text-[10px] font-bold uppercase tracking-widest text-[#e5ad46] hover:bg-[#e5ad46]/10"><ImagePlus className="h-4 w-4" />{uploading === field.key ? "Import…" : "Importer"}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => upload(field, event)} className="sr-only" disabled={uploading !== null} /></label><button type="button" onClick={() => resetImage(field)} className="inline-flex min-h-10 items-center gap-2 rounded-lg px-3 text-[10px] font-bold uppercase tracking-widest text-[#eccc90]/60 hover:bg-white/5 hover:text-[#eccc90]"><RefreshCcw className="h-3.5 w-3.5" />Image par défaut</button></div><p className="text-xs text-[#eccc90]/45">L’image importée est conservée sur le serveur; seul son lien est publié.</p></div></div> : field.multiline ? <textarea value={value(field)} onChange={(event) => setValue(field.key, event.target.value)} rows={4} className="w-full resize-y rounded-xl border border-[#e5ad46]/20 bg-[#1e2a38] px-3 py-2.5 text-sm leading-relaxed text-[#eccc90] outline-none focus:border-[#e5ad46]" /> : <><input value={value(field)} onChange={(event) => setValue(field.key, event.target.value)} className="w-full rounded-xl border border-[#e5ad46]/20 bg-[#1e2a38] px-3 py-2.5 text-sm text-[#eccc90] outline-none focus:border-[#e5ad46]" />{field.hint && <small className="mt-1 block text-xs text-[#eccc90]/45">{field.hint}</small>}</>}</div>)}</div></details>)}</div>}
