    ...
    <div className="p-6 md:p-10 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Wrench className="h-6 w-6 text-[#FFB31B]" />
          <h1 className="font-headline text-2xl text-[#FFB31B]">Tickets Réparation Machine</h1>
        </div>
      ...
      {showForm && (
        <div className="rounded-xl bg-[#25303a] p-5 border border-[#FFB31B]/20 space-y-4">
          <input value={form.machine} onChange={(e) => setForm({ ...form, machine: e.target.value })} placeholder="Machine concernée" className="w-full rounded-lg bg-[#1e2a38] px-4 py-3 text-sm text-[#FFB31B] placeholder:text-[#FFB31B]/30 outline-none border border-[#FFB31B]/10 focus:border-[#FFB31B]/30" />
          <input value={form.reportedBy} onChange={(e) => setForm({ ...form, reportedBy: e.target.value })} placeholder="Signalé par" className="w-full rounded-lg bg-[#1e2a38] px-4 py-3 text-sm text-[#FFB31B] placeholder:text-[#FFB31B]/30 outline-none border border-[#FFB31B]/10 focus:border-[#FFB31B]/30" />
          <button onClick={addTicket} className="w-full rounded-lg bg-[#FFB31B] py-3 text-[10px] font-bold uppercase tracking-widest text-[#1e2a38] transition hover:bg-[#FFB31B]">Créer le ticket</button>
        </div>
      )}
      ...
      {filteredTickets.length === 0 ? (
        <p className="text-sm text-[#FFB31B]/50">{statusFilter ? "Aucun ticket avec ce statut." : "Aucun ticket de réparation."}</p>
      ) : (
        <div className="space-y-3">
          {filteredTickets.map((t) => (
            <div key={t.id} className="rounded-xl bg-[#25303a] p-4 border border-[#FFB31B]/10 flex items-center justify-between cursor-pointer hover:bg-[#2a3642] transition-colors" onClick={() => toggleStatus(t.id, t.status)}>
              <div className="flex items-center gap-3">
                <span className={`h-2.5 w-2.5 rounded-full ${t.status === "ouvert" ? "bg-red-400" : t.status === "en cours" ? "bg-yellow-400" : "bg-green-400"}`} />
                <div>
                  <p className="font-semibold text-[#FFB31B]">{t.machine}</p>
                  <p className="text-xs text-[#FFB31B]/50">Signalé par {t.reportedBy} • {t.date}</p>
                </div>
              </div>
