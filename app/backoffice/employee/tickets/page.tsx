    ...
    <div className="p-6 md:p-10 space-y-6">
      ...
      {showForm && (
        <div className="rounded-xl bg-[#25303a] p-5 border border-[#EAA100]/20 space-y-4">
          ...
          <button onClick={addTicket} className="w-full rounded-lg bg-[#EAA100] py-3 text-[10px] font-bold uppercase tracking-widest text-[#1e2a38] transition hover:bg-[#EAA100]">Créer le ticket</button>
        </div>
      )}
      ...
      {filteredTickets.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredTickets.map((t) => (
            <div key={t.id} className="rounded-xl bg-[#25303a] p-4 border border-[#EAA100]/10 flex items-center justify-between cursor-pointer hover:bg-[#2a3642] transition-colors" onClick={() => toggleStatus(t.id, t.status)}>
              <div className="flex items-center gap-3">
