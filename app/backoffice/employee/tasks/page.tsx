  ...
  return (
    <div className="p-6 md:p-10 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-headline text-2xl text-[#F5A623]">Suivi Employés</h1>
      </div>
      ...
      {loading ? (
        <div className="flex justify-center py-8"><LoaderIcon className="h-5 w-5 animate-spin text-[#F5A623]" /></div>
      ) : employees.length === 0 ? (
        <p className="text-sm text-[#F5A623]/50">Aucun employé inscrit.</p>
      ) : filteredEmployees.length === 0 ? (
        <p className="text-sm text-[#F5A623]/50">Aucun employé avec ce statut.</p>
      ) : (
        <div className="space-y-3">
          {filteredEmployees.map((e) => (
            <div key={e.id} className="rounded-xl bg-[#25303a] p-4 border border-[#F5A623]/10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className={`h-3 w-3 rounded-full ${e.present ? "bg-green-400" : "bg-red-400"}`} />
                <div>
                  <p className="font-semibold text-[#F5A623]">{e.first_name} {e.last_name}</p>
                  <p className="text-xs text-[#F5A623]/50">{e.email}</p>
                </div>
              </div>
              <button
