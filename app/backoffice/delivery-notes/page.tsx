  ...
  return (
    <div className="px-6 md:px-12 py-10 space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="font-headline text-3xl text-[#EAA100] mb-2">Bons de Livraison</h1>
          <p className="text-xs uppercase tracking-widest text-[#EAA100]/40 font-bold">Suivi des expéditions atelier</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-[#25303a] p-6 rounded-[2rem] border border-[#EAA100]/10 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <input required placeholder="N° Bon de livraison" value={formData.numero} onChange={(e) => setFormData({...formData, numero: e.target.value})} className="w-full px-4 py-3 bg-[#1e2a38] border border-[#EAA100]/10 rounded-xl text-sm" />
            <input type="date" required value={formData.date_livraison} onChange={(e) => setFormData({...formData, date_livraison: e.target.value})} className="w-full px-4 py-3 bg-[#1e2a38] border border-[#EAA100]/10 rounded-xl text-sm" />
            <input required placeholder="Destinataire" value={formData.destinataire} onChange={(e) => setFormData({...formData, destinataire: e.target.value})} className="w-full px-4 py-3 bg-[#1e2a38] border border-[#EAA100]/10 rounded-xl text-sm" />
      ...
      <div className="bg-[#25303a] rounded-[2.5rem] border border-[#EAA100]/10 shadow-sm overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center text-sm text-[#EAA100]/40 italic">Aucun bon de livraison.</div>
        ) : (
          ...
          >
            <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-[#EAA100]/10 bg-[#1e2a38]/95 px-6 py-5 backdrop-blur md:px-10 md:py-7">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setSelectedId(null)} aria-label="Fermer le détail" className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-[#EAA100]/10 text-[#EAA100]/60 transition hover:border-[#EAA100]/10 hover:bg-[#163526] hover:text-white focus:outline-none focus:ring-2 focus:ring-[#EAA100]">
                  <X className="h-5 w-5" />
                </button>
              </div>
            </header>
