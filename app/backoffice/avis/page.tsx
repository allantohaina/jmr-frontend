  ...
  return (
    <div className="px-6 md:px-12 py-10 space-y-8">
      <div className="flex justify-between items-end px-2">
        <div>
          <h2 className="font-headline text-3xl text-[#FFB31B]">Avis clients</h2>
          <p className="text-[#FFB31B]/40 text-xs font-bold uppercase tracking-widest mt-1">Modération · JMR Atelier</p>
        </div>
        <div className="flex gap-2">
          {FILTERS.map((f) => (
            <button key={f.key} onClick={() => setFilter(f.key)} className={`rounded-lg px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest transition-colors ${filter === f.key ? "bg-[#163526] text-[#FFB31B]" : "border border-[#FFB31B]/15 text-[#FFB31B] hover:border-[#FFB31B]"}`}>
              {f.label}
            </button>
          ))}
        </div>
      ...
      <div className="bg-[#25303a] rounded-[2rem] overflow-hidden shadow-sm border border-[#FFB31B]/10">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-sm text-[#FFB31B]/40 italic">Aucun avis pour ce filtre.</div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="text-sm font-bold text-[#FFB31B]">{a.auteur || "Anonyme"}</span>
                      <span className="text-[#FFB31B] text-sm tracking-widest">{stars(a.note)}</span>
                    ...
                    <p className="text-xs text-[#FFB31B]/70">{a.commentaire || "—"}</p>
                    <p className="text-[10px] text-[#FFB31B]/40 mt-1">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {a.statut !== "pending" && (
                      <button onClick={() => setStatut(a.id, "pending")} className="rounded-lg border border-[#FFB31B]/15 px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest text-[#FFB31B] hover:border-[#FFB31B]">
                        Revenir en attente
                      </button>
                    )}
                  </div>
