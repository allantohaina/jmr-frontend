...
function NavGroup({ title, icon: GroupIcon, items }: { title: string; icon: React.ElementType; items: { href: string; label: string; icon: React.ElementType }[] }) {
  ...
  return (
    <div className="mb-[2px]">
      ...
      {open && (
        <div className="ml-5 space-y-[1px] border-l border-dashed border-[rgba(234, 161, 0,0.14)] pl-2.5">
          {items.map(({ href, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                {active && (
                  <span className="absolute -left-[11px] top-[3px] bottom-[3px] w-[2px] rounded-sm bg-[#EAA100]" />
                )}
                <Icon className={`h-[15px] w-[15px] shrink-0 ${active ? "text-[#EAA100] opacity-100" : "opacity-80"}`} />
                {label}
              </Link>
            );
          })}
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#1e2a38] font-body text-[#EAA100] md:flex">
      <aside className="z-50 flex w-full flex-col bg-[linear-gradient(180deg,#161D30,#0A0E19)] shadow-2xl transition-all duration-300 md:fixed md:left-0 md:top-0 md:h-full md:w-64 border-r border-white/5">
        ...
        <div className="mx-6 mb-[6px] border-b border-dashed border-[rgba(234, 161, 0,0.14)]" />
        ...
        <nav className="hidden flex-1 space-y-0 overflow-y-auto px-[14px] py-[10px] md:block">
          ...
          >
            ...
            <LayoutDashboard className={`h-[15px] w-[15px] shrink-0 ${pathname === "/backoffice" ? "text-[#EAA100] opacity-100" : "opacity-80"}`} />
            Tableau de bord
          </Link>
