  ...
  return (
    <div className="p-6 md:p-10 space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-headline text-2xl text-[#FFB42D]">Factures proforma</h1>
          <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-[#FFB42D]/40">
      ...
      {loading ? (
        <div className="flex justify-center py-10"><Loader2 className="h-5 w-5 animate-spin text-[#FFB42D]" /></div>
      ) : rows.length === 0 ? (
          ...
          {rows.map((r) => (
            <button key={r.id} onClick={() => openRow(r.id)} className="pf-row">
              <div className="min-w-0">
                <p className="font-bold text-[#FFB42D]">{r.number}</p>
                <p className="text-xs text-[#FFB42D]/55 truncate">{r.client_name} · {r.client_email || "—"}</p>
              </div>
              <div className="text-right shrink-0 ml-4">
                <p className="font-bold text-[#FFB42D]">{formatMoney(Number(r.total ?? 0), r.currency)}</p>
      ...
      <style jsx>{`
        .pf-btn { display: inline-flex; align-items: center; gap: 8px; background: #FFB42D; color: #1e2a38; border: 0; border-radius: 12px; padding: 10px 18px; font-size: 11px; font-weight: 800; letter-spacing: .08em; text-transform: uppercase; cursor: pointer; }
        .pf-btn:disabled { opacity: .5; cursor: not-allowed; }
        .pf-btn.ghost { background: transparent; border: 1px solid rgba(255,180,45,.3); color: #FFB42D; }
        .pf-btn.ghost.danger { border-color: rgba(255,80,80,.4); color: #ff8080; }
        .pf-icon { display: grid; place-items: center; width: 38px; height: 38px; border-radius: 10px; border: 1px solid rgba(255,180,45,.2); background: transparent; color: #FFB42D; cursor: pointer; }
        ...
        .pf-chip { padding: 8px 14px; border-radius: 999px; font-size: 10px; font-weight: 800; letter-spacing: .1em; text-transform: uppercase; background: #25303a; color: rgba(255,180,45,.6); border: 1px solid rgba(255,180,45,.12); cursor: pointer; }
        .pf-chip.active { background: #FFB42D; color: #1e2a38; border-color: #FFB42D; }
        .pf-search { display: flex; align-items: center; gap: 8px; background: #25303a; border: 1px solid rgba(255,180,45,.15); border-radius: 12px; padding: 8px 12px; color: rgba(255,180,45,.5); margin-left: auto; }
        .pf-search input { background: transparent; border: 0; outline: none; color: #FFB42D; font-size: 13px; width: 200px; }
        .pf-row { width: 100%; text-align: left; display: flex; justify-content: space-between; align-items: center; background: #25303a; border: 1px solid rgba(255,180,45,.1); border-radius: 14px; padding: 14px 18px; cursor: pointer; }
        .pf-row:hover { border-color: rgba(255,180,45,.35); }
        .pf-status { font-size: 9px; text-transform: uppercase; letter-spacing: .14em; color: rgba(255,180,45,.45); margin: 2px 0 0; }
        .pf-empty { background: #25303a; border: 1px dashed rgba(255,180,45,.15); border-radius: 16px; padding: 40px 24px; text-align: center; color: rgba(255,180,45,.5); font-size: 14px; }
        ...
        .pf-sheet { background: #141d2b; border: 1px solid rgba(255,180,45,.15); border-radius: 20px; padding: 24px; width: 100%; max-width: 760px; }
        .pf-sheet.wide { max-width: 960px; }
        .pf-sheet.small { max-width: 520px; }
        .pf-sheet-head { display: flex; justify-content: space-between; align-items: flex-start; gap: 12px; margin-bottom: 18px; color: #FFB42D; }
        .pf-sheet-head h2 { margin: 4px 0 0; font-size: 16px; }
        .pf-sheet-head button { background: transparent; border: 1px solid rgba(255,180,45,.2); color: #FFB42D; border-radius: 10px; width: 36px; height: 36px; display: grid; place-items: center; cursor: pointer; }
        .pf-kicker { margin: 0; font-size: 9px; font-weight: 800; letter-spacing: .22em; text-transform: uppercase; color: rgba(255,180,45,.5); }
        .pf-toolbar { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 16px; }
        .pf-select { width: 100%; background: #1e2a38; border: 1px solid rgba(255,180,45,.2); border-radius: 10px; color: #FFB42D; padding: 12px; font-size: 14px; }
        @media print {
...
export default function ProformasPage() {
  return (
    <Suspense fallback={<div className="p-10 flex justify-center"><Loader2 className="h-5 w-5 animate-spin text-[#FFB42D]" /></div>}>
      <ProformasContent />
    </Suspense>
  );
}
