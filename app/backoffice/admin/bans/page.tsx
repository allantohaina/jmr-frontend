  ...
  return (
    <div className="space-y-8 px-6 py-10 md:px-12">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-headline text-2xl text-[#FFB42D]">Gestion des Bannissements</h2>
          <p className="mt-1 text-sm text-[#FFB42D]/60">Bannir ou réhabiliter des utilisateurs</p>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-md">
        ...
        <input
          ...
          className="w-full rounded-xl border border-[#FFB42D]/20 bg-[#25303a] py-3 pl-10 pr-4 text-sm text-[#FFB42D] placeholder-[#FFB42D]/30 outline-none transition-colors focus:border-[#FFB42D]"
        />
      </div>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-[#FFB42D]/5">
            {filteredUsers.map(user => {
              ...
              return (
                <tr key={user.id} className="transition-colors hover:bg-[#FFB42D]/5">
                  <td className="px-6 py-4 font-medium text-[#FFB42D]">
                    {user.first_name} {user.last_name}
                  </td>
                  <td className="px-6 py-4 text-[#FFB42D]/70">{user.email}</td>
                  <td className="px-6 py-4">
                    <span className="rounded-full bg-[#FFB42D]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[#FFB42D]">
                      {user.role}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    {isBanned ? (
                      <span className="flex items-center gap-2 text-red-400">
                        ...
                        {userBan?.expires_at && (
                          <span className="text-[10px] text-[#FFB42D]/40">
                  ...
                  <td className="px-6 py-4">
                    {isBanned ? (
                      ...
                      <span className="text-[10px] text-[#FFB42D]/30">—</span>
                    )}
                  </td>
                </tr>
              );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-[#FFB42D]/20 bg-[#1e2a38] p-8 shadow-2xl">
            <div className="mb-6 flex items-center gap-3">
              ...
              <h3 className="font-headline text-lg text-[#FFB42D]">Bannir un utilisateur</h3>
            </div>
            <p className="mb-6 text-sm text-[#FFB42D]/70">
              Êtes-vous sûr de vouloir bannir <strong className="text-[#FFB42D]">{banModal.user.first_name} {banModal.user.last_name}</strong> ({banModal.user.email}) ?
            </p>
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-[10px] font-bold uppercase tracking-widest text-[#FFB42D]/60">Motif du ban</label>
                <textarea
                  value={banReason}
              ...
              <div>
                <label className="mb-1 block text-[10px] font-bold uppercase tracking-widest text-[#FFB42D]/60">
