  ...
  return (
    <div className="px-6 md:px-12 py-10 space-y-8">
      <div className="flex justify-between items-end px-2">
        <div>
          <h2 className="font-headline text-3xl text-[#EAA100]">Matières premières</h2>
          <p className="text-[#EAA100]/40 text-xs font-bold uppercase tracking-widest mt-1">Stock atelier · JMR</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-[#161D30] rounded-2xl border border-[#EAA100]/10 p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <select value={form.unite} onChange={(e) => setForm({ ...form, unite: e.target.value })} className="rounded-xl border border-[#EAA100]/15 px-4 py-2.5 text-sm text-[#EAA100]">
              {["m", "kg", "pce", "L", "rouleau"].map((u) => <option key={u} value={u}>{u}</option>)}
            </select>
            <input value={form.fournisseur} onChange={(e) => setForm({ ...form, fournisseur: e.target.value })} placeholder="Fournisseur" className="rounded-xl border border-[#EAA100]/15 px-4 py-2.5 text-sm text-[#EAA100]" />
            <input type="number" value={form.stock_actuel} onChange={(e) => setForm({ ...form, stock_actuel: e.target.value })} placeholder="Stock actuel" className="rounded-xl border border-[#EAA100]/15 px-4 py-2.5 text-sm text-[#EAA100]" />
            <input type="number" value={form.stock_seuil} onChange={(e) => setForm({ ...form, stock_seuil: e.target.value })} placeholder="Seuil d'alerte" className="rounded-xl border border-[#EAA100]/15 px-4 py-2.5 text-sm text-[#EAA100]" />
            <input type="number" value={form.prix_unite} onChange={(e) => setForm({ ...form, prix_unite: e.target.value })} placeholder="Prix unitaire" className="rounded-xl border border-[#EAA100]/15 px-4 py-2.5 text-sm text-[#EAA100]" />
      ...
      <div className="bg-[#161D30] rounded-[2rem] overflow-hidden shadow-sm border border-[#EAA100]/10">
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center text-sm text-[#EAA100]/40 italic">Aucune matière enregistrée.</div>
        ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0A0E19]/70 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setDetail(null); }}>
          <div className="relative w-full max-w-lg bg-[#161D30] rounded-2xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-4">
              ...
              <button onClick={() => setDetail(null)} className="text-[#EAA100]/40 hover:text-[#EAA100]">✕</button>
            </div>
